...
void *pthreadXinTiao (void *arg)
{
	...
	while(g_ProtocolStart)
	{
		/*初始化源mac*/
		while (SourceMacNoInit == CheckInitSourceMac())
		{
			//TRACE("sourcemac init begin\n");
			InitSourceMac();
		...
		while(1 == g_ProtocolStart && SourceMacInited == CheckInitSourceMac())
		{
			sleep(FRE_XINTIAO);
...
int CheckAA55(ISmartFrame *pISMartFrame)
{
	...
	{
		TRACEERR("CheckAA55 fail:%02x %02x \n", pISMartFrame->heda[0], pISMartFrame->heda[1]);
		return ReturnError;
	}
	return ReturnSuccess;
